...
class GAMITree:
    def __init__(
        ...
        **params,
        ...
        self.log = log if log is not None else (lambda s: print(s, flush=True))  # noqa: T201
        self.base: float = 0.0
    ...
    def main_component_round(self, X: np.ndarray, j: int, r: int) -> np.ndarray:
        ...
        main_end = (
            len(self.main_trees)
            if r <= 0 or not self.rounds
            else self.rounds[r - 1].main_end
        )
        pair_splines = (
            self.pair_splines
            if r <= 0 or not self.rounds
            else self.rounds[r - 1].pair_splines
        )
    ...
    def interaction_component_round(
        self, X: np.ndarray, j: int, k: int, r: int
    ) -> np.ndarray:
        X = np.asarray(X, float)
    ...
    def interaction_component(self, X: np.ndarray, j: int, k: int) -> np.ndarray:
        return self.interaction_component_round(
            X, j, k, r=len(self.rounds) if self.rounds else 0
        )
    ...
    def fit_main_stage(
        ...
        losses: list[float] = []
        loop = self.iter_wrap(
            range(1, M + 1), desc=f"Round {r}/{R} FitMain", unit="tree", leave=False
        )
        for m in loop:
            ...
            if hasattr(loop, "set_postfix"):
                loop.set_postfix(
                    {"m": m, "main": len(self.main_trees), "val": f"{Lm:.4f}"}
                )
            if len(losses) >= d + 1:
                L_m_d = losses[-(d + 1)]
    ...
    def filter_interactions(
        self, X: np.ndarray, y: np.ndarray, tr: np.ndarray, r: int, R: int
    ) -> list[tuple[int, int]]:
        p = int(X.shape[1])
        ...
        for j in range(p):
            trj = SplineTransformer(
                degree=1,
                n_knots=int(self.params.nknots),
                knots="quantile",
                include_bias=False,
            )
        ...
        loop = self.iter_wrap(
            pairs0, desc=f"Round {r}/{R} FilterInt", unit="pair", leave=False
        )
                ...
                loop.set_postfix(
                    {
                        "done": idx,
                        "topk": len(pairs0),
                        "best_sse": f"{min(s[0] for s in scores):.2f}",
                    }
                )
    ...
    def fit_int_stage(
        ...
        for j, _k in Q:
            if int(j) not in trs:
                ...
                trs[int(j)] = trj
        loop = self.iter_wrap(
            range(1, M + 1), desc=f"Round {r}/{R} FitInt", unit="tree", leave=False
        )
        for m in loop:
            G, H, _ = loss_derivatives(self.task, y[tr], f_tr)
            z = pseudo_response(G, H)
            ...
            if hasattr(loop, "set_postfix"):
                loop.set_postfix(
                    {"m": m, "int": len(self.int_trees), "val": f"{Lm:.4f}"}
                )
            if len(losses) >= d + 1:
                L_m_d = losses[-(d + 1)]
                if L_m_d < min(losses[-d:]):
    ...
    def purify_from_state(
        self, X: np.ndarray, tr: np.ndarray, int_end: int
    ) -> dict[tuple[int, int], tuple[Spline1D, Spline1D]]:
        if int_end <= 0:
            return {}
        ...
        for a, b in loop:
            ...
            tr_a = SplineTransformer(
                degree=1,
                n_knots=int(self.params.nknots),
                knots="quantile",
                include_bias=False,
            )
            tr_b = SplineTransformer(
                degree=1,
                n_knots=int(self.params.nknots),
                knots="quantile",
                include_bias=False,
            ...
            beta, _sse = fit_leaf(
                Phi,
                yhat,
                np.ones(Xt.shape[0], float),
                self.params.alpha_grid,
                float(self.params.max_coef),
            )
            na = Ba.shape[1]
            ga = beta[: 1 + na]
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> GAMITree:
        ...
        y = np.asarray(y, float).reshape(-1)
        n = int(X.shape[0])
        tr, va = train_valid_split(
            n, float(self.params.valid_frac), int(self.params.seed)
        )
        ...
        for r in range(1, R + 1):
            ...
            self.rounds.append(
                RoundSnapshot(
                    main_end=main_end, int_end=int_end, pair_splines=pair_splines_r
                )
            )
            if m1 == 0 and m2 == 0:
                break
